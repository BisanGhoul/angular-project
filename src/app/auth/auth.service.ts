...
export class AuthService{
    ...
    autoLogin(){
        ...
        if(loadedUser.token){ //will check if the token is valid, by calling its getter where we check if the date is expired
            ...
            const expirartionDuration = new Date(userData._tokenExpirationDate).getTime()
             - new Date().getTime();
            this.autoLogout(expirartionDuration);
        }
    }
    ...
    logout(){
        ...
        if(this.tokenExpirationTimer){// if active
            clearTimeout(this.tokenExpirationTimer); //clear
        }
    ...
    private handleAuthentication(email:string, userId:string, token:string, expiresIn: number){
            ...
            this.autoLogout(expiresIn * 1000); // bcuz we need it in millies
